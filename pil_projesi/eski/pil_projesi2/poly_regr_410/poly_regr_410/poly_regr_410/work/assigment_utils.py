...
def plt_train_test(X_train, y_train, X_test, y_test, x, y_pred, x_ideal, y_ideal, degree):
    ...
    ax.set_title("Performance on Test Data",fontsize = 12)
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ...
    ax.plot(x, y_pred,  lw=1, label=f"predicted, degree={degree}")
...
def plt_optimal_degree(X_train, y_train, X_cv, y_cv, x, y_pred, x_ideal, y_ideal, err_train, err_cv, optimal_degree, max_degree):
    ...
    ax[0].set_ylabel("Voltage Level [V]")
    ax[0].grid()

    ax[0].plot(x_ideal, y_ideal, "--", color = "orangered",  label="y_ideal", lw=1)
    ax[0].scatter(X_train, y_train, color = "green",         label="train")
    ...
    ax[1].axvline(optimal_degree, lw=2, color = dlc["dlmagenta"])
...
def plt_tune_m(X_train, y_train, X_cv, y_cv, x, y_pred, err_train, err_cv, m_range, degree):
    ...
    ax[0].scatter(X_train, y_train, color = "green",           s=3, label="train", alpha=0.4)
